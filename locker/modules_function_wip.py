...
def nearestbicimad_input(title):
    ...
    print(f'Your nearest station is {data.data_bicimad["name"][dist.index(min(dist))].split("- ")[1]} located at {round((min(dist)/1000), 2)} km from you and has {data.data_bicimad["dock_bikes"][dist.index(min(dist))]} bikes and {data.data_bicimad["free_bases"][dist.index(min(dist))]} free bases available. The CSV is ready in the Output folder.')
