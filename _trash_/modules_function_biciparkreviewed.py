...
def nearestbicimad_input(title):
    ...
    print(f'Your nearest station is {data.data_bicimad["name"][dist.index(min(dist))].split("- ")[1]} located at {round((min(dist)/1000), 2)} km from you and has {api.stations_api_data["dock_bikes"][number_api]} bikes and {api.stations_api_data["free_bases"][number_api]} free bases available. The CSV is ready in the Output folder.')
...
def nearestbicipark_input(title):
    ...
    print(f'Your nearest station is {data.data_bicipark["stationName"][dist.index(min(dist))]} located at {round((min(dist)/1000), 2)} km from you and has {data.data_bicipark["free_places"][dist.index(min(dist))]} free bases available. The CSV is ready in the Output folder.')
